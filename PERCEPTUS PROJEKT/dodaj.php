...
if ($polaczenie->connect_errno != 0) {
	...
	$kod = $_GET["kod"];
	$ile = "SELECT email FROM dodaj WHERE kod='$kod'";
	...
	if(isset($_POST["password"])){
		...
		$email = $row["email"];
		$kon = "INSERT into uzytkownicy (email,haslo,typ,autoryzacja,kosz,mb) VALUES ('$email',SHA2('$pw',512),'uzytkownik',0,30,16777215)";
		$konn = @$polaczenie->query($kon);
		if($konn){
			exit("Utworzono konto");
		}
		else{
...
?>	
	...
	<div class="login-container">
		...
		<form method="post">
				...
				<br><input type="submit" id="submit" value="Stwórz konto">
			</form>
		</div>
	</body>
	</html>